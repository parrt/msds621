...
def run_regression_test(X, y, ntrials=2, min_training_score = .90):
    ...
    for i in range(ntrials):
        ...
        sklearn_rf = RandomForestRegressor(n_estimators=15, min_samples_leaf=3, max_features='sqrt')
...
def run_classification_test(X, y, ntrials=1):
    X = X[:500]
    y = y[:500]
    scores = []
    train_scores = []
    ...
    for i in range(ntrials):
        ...
        sklearn_rf = RandomForestClassifier(n_estimators=20, min_samples_leaf=3, max_features='sqrt')
        sklearn_rf.fit(X_train, y_train)
        sklearn_score = sklearn_rf.score(X_train, y_train)
        sklearn_train_scores.append(sklearn_score)
        sklearn_score = sklearn_rf.score(X_test, y_test)
    ...
    assert np.mean(train_scores) > 0.95, \
           f"Training accuracy: {np.mean(train_scores):.2f} must 0.95"
